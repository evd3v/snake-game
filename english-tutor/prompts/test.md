...
- pair: проверка на различение пары. Предложение с пропуском «_____», в котором по контексту подходит ТОЛЬКО «{{headword}}», а его близнец по переводу звучал бы неестественно или менял смысл. В самом конце в скобках два варианта через « / » в случайном порядке: «{{headword}}» и близнец из данных (поле twin). Слова из вариантов внутри предложения быть не должно. answer: «{{headword}}».
